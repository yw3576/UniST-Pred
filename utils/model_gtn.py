...
class GTN(nn.Module):
    ...
    def __init__(self, num_edge, num_channels, w_in, w_out, num_class, num_nodes, num_layers, args=None):
        ...
        #self.act = nn.Sigmoid()
        #self.mean_ = nn.Parameter(torch.zeros(num_edge))
        #self.std_ = nn.Parameter(torch.ones(num_edge))
    ...
    def forward(self, A, X, target_x, num_nodes=None, eval=False, node_labels=None):
        ...
        for i in range(self.num_channels):
            ...
            if i==0:                
                X_ = self.gcn(X,edge_index=edge_index.detach().type(torch.int64), edge_weight=edge_weight)
                X_ = F.relu(X_)
            else:
        ...
        #print(X_)
        #print(X_.size())
        y = self.linear(X_[target_x])# + torch.normal(self.mean_, self.act(self.std_)*0.1)
...
class GTConv(nn.Module):
    ...
    def forward(self, A, num_nodes, eval=eval):
        ...
        for i in range(num_channels):
            ...
            index, value = torch_sparse.coalesce(total_edge_index.detach(), total_edge_value, m=num_nodes-2, n=num_nodes, op='add')
            #index, value = torch_sparse.coalesce(total_edge_index.detach(), total_edge_value, op='add')
