...
class GCNConv(MessagePassing):
    ...
    def norm(edge_index, num_nodes, edge_weight, improved=False, dtype=None, args=None):
        ...
        deg = scatter_add(edge_weight, row.type(torch.int64), dim=0, dim_size=num_nodes)
        deg_inv_sqrt = deg.pow(-1)
        deg_inv_sqrt[deg_inv_sqrt == float('inf')] = 0

        # return edge_index, (deg_inv_sqrt[col] ** 0.5) * edge_weight * (deg_inv_sqrt[row] ** 0.5)
        return edge_index, deg_inv_sqrt[row.type(torch.int64)] * edge_weight
    ...
    def forward(self, x, edge_index, edge_weight=None):
        ...
        return self.propagate(edge_index.type(torch.int64), x=x, norm=norm)
